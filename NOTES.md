...
- **9/21 22:5x 版面修正**：賢賢看了 Blogger 版說「第一張要放大、第二張要拉長、兩兄弟字體要一樣大」。兩頁 `#lb-story` 改成滿版破格（`width:100vw;margin-left:calc(50% - 50vw)`，內層最寬 1120），桌機量到漫畫區 0～1280 滿寬、手機 390 無橫捲；側欄橫幅改直式：黑底卡、兩顆等大按鈕（藍聖翔／紅聖宸）＋貼圖頭像，小工具標題清空避免重複。更新走 CodeMirror API：`document.querySelector(".CodeMirror").CodeMirror.setValue(html)` 再打一個空白退格讓「更新」鈕亮起；驗證要看線上網址不要看隱藏 textarea。
